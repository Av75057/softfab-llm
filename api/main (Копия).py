...
async def admin_page():
    ...
    # Статус backend из глобальных переменных
    now = datetime.datetime.utcnow()
    status = last_status
    ...
    # Формируем строки таблицы для последних N (например, 20) записей
    table_rows = ""
    for rec in reversed(records[-20:]):  # последние 20 по времени
        ...
        # Нестримовый путь: стандартный OpenAI-формат
        choices = resp.get("choices")
        if isinstance(choices, list) and choices:
            msg = choices[0].get("message", {}) or {}
            reply_preview = msg.get("content", "")

        # Стримовый путь: мы логировали {"stream": True, "full_reply": "..."}
    ...
    <body>
      <header>
        ...
        <div>
          <span>Текущее время (UTC): {html.escape(now.isoformat(timespec="seconds"))}Z</span>
        </div>
      </header>
      <main>
        <div class="card">
          ...
          <p class="small">Для подробного статуса можно открыть <a href="/">страницу статуса</a> или <a href="/backend/health">/backend/health</a>.</p>
        </div>
        ...
        <div class="card">
          <h2>Последние запросы (за сегодня, максимум 20)</h2>
            ...
            <table>
              <thead>
                <tr>
                  <th>Время (UTC)</th>
                  <th>HTTP</th>
                  <th>Пользовательский запрос</th>
                  <th>Ответ модели</th>
                </tr>
              </thead>
              <tbody>
                {table_rows or '<tr><td colspan="4">Нет записей в логах за сегодня.</td></tr>'}
              </tbody>
            </table>
          </div>
          <p class="small">Берутся последние строки из файла логов за текущую дату.</p>
        </div>
      </main>
    </body>
    </html>
...
<body>
  ...
  <script>
    ...
    const ROLE_PRESETS = {
      default: {
        ...
        system:
          ...
          "Ты универсальный ассистент. Отвечай строго на русском языке. " +
          "Никогда не используй китайский язык (иероглифы) ни в каких частях ответа. " +
          "Если запрос приходит на другом языке — мысленно переведи его, а отвечай по-русски. " +
      ...
      coder: {
        ...
        system:
          ...
          "Твоя задача — помогать писать и разбирать код, предлагать улучшения, находить ошибки. " +
          "Сначала кратко объясняй идею, затем приводи конкретный код с комментариями на русском. " +
          "Если есть несколько вариантов решения, перечисли их с плюсами и минусами."
      },
      architect: {
        name: "Системный архитектор",
        description: "Архитектура, инфраструктура, интеграции, high-level дизайн.",
        system:
          "Ты системный архитектор с большим опытом в проектировании масштабируемых и отказоустойчивых систем. " +
          "Отвечай строго на русском языке. " +
          "Структурируй ответы: сначала краткое резюме, затем детальная архитектура по слоям и компонентам, " +
          "затем риски и варианты масштабирования. Учитывай практические аспекты: мониторинг, логирование, CI/CD, безопасность."
      },
      math: {
        name: "Математик / теоретик",
        description: "Решение и разбор математических задач, теории, доказательства.",
        system:
          "Ты математик-теоретик. Отвечай строго на русском языке. " +
          "Объясняй решения пошагово, начиная с переформулировки задачи. " +
          "Если есть несколько способов решения, укажи их. " +
          "Избегай излишней формальности там, где можно объяснить проще, но не жертвуй корректностью."
      },
      translator: {
        name: "Переводчик",
        description: "Перевод текстов, адаптация стиля, пояснения значений.",
        system:
          "Ты переводчик и редактор. Основной язык ответа — русский. " +
          "Переводи тексты с сохранением смысла и стиля, но делай формулировки естественными для носителя русского языка. " +
          "При необходимости давай краткие комментарии по выбору формулировки и нюансам."
      }
    };
    ...
    const conversation = [
      {
        role: "system",
        content: ROLE_PRESETS[currentRoleKey].system
      }
    ];
    ...
    roleSelectEl.addEventListener("change", () => {
      ...
      updateRoleInfo();
      const text = "Роль изменена на: " + ROLE_PRESETS[currentRoleKey].name;
    ...
    function resetConversation() {
      // сбрасываем контекст на текущую роль
      ...
      // очищаем все сообщения, кроме строки с ролью
